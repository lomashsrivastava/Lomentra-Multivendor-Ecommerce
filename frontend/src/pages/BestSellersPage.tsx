...
import { TrendingUp, Star, ShoppingCart, Heart, Award } from 'lucide-react'
...
export function BestSellersPage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      setLoading(true)
      try {
        const r = await fetch('/api/products?limit=24')
        if (r.ok) {
          const d = await r.json()
          // Sort items by rating (in demo we sort by title length or mock rating for high-tech list)
          const items = (d.products || []).map((p: any, idx: number) => ({
            ...p,
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <span className="text-[10px] uppercase tracking-widest font-black text-primary flex items-center gap-1.5">
              <Award className="h-3.5 w-3.5 text-primary" /> Top Rated
            </span>
            <h1 className="text-2xl md:text-3xl font-black text-foreground">Best Sellers</h1>
            <p className="text-xs text-muted-foreground max-w-xl">
              The most popular, highly-reviewed items loved by our tech community. Quality guaranteed.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((p, index) => {
            ...
            return (
              ...
              >
                {/* Ranking Badge */}
                <div className="absolute top-3 left-3 z-10 flex h-7 w-7 items-center justify-center rounded-lg bg-primary text-[11px] font-black text-primary-foreground shadow-md shadow-primary/20">
                  #{rank}
                </div>

                <div className="relative h-44 bg-muted/20 overflow-hidden">
                  <img
                    src={p.images[0] || 'https://images.unsplash.com/photo-1572635196237-14b3f281503f?w=350'}
                    alt={p.name}
                    ...
                    <div className="flex text-amber-400">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`h-3 w-3 ${
                            i < Math.floor(p.rating) ? 'fill-amber-400' : 'text-muted/30'
                          }`}
                        />
                      ))}
                    </div>
                    <span className="text-[10px] font-extrabold text-foreground">{p.rating}</span>
                    <span className="text-[9px] text-muted-foreground">({p.reviewsCount})</span>
                  </div>
